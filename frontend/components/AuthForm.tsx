    ...
    <section className="auth-form">
      ...
      {user ? (
        ...
        <>
          ...
          <div className="relative rounded-xl overflow-hidden cursor-pointer" style={{ height: '44px' }}>
            {/* Custom violet visual layer */}
            <div className={`absolute inset-0 flex items-center justify-center gap-3 bg-gradient-to-r from-violet-600 to-cyan-600 text-white font-semibold text-sm pointer-events-none z-10 rounded-xl ${backendCold ? 'opacity-50' : ''}`}>
              {isGoogleLoading ? (
            ...
            {/* Invisible Google iframe on top — captures clicks */}
            {!backendCold && (
              <div
                ref={googleBtnRef}
                className="absolute inset-0 z-20 opacity-0 overflow-hidden [&_iframe]:!w-full [&_iframe]:!h-[44px]"
              />
            )}
          </div>
